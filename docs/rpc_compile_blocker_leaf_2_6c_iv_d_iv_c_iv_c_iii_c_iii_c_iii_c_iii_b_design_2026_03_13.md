# RPC Compile Blocker Leaf 2.6.c.iv.d.iv.c.iv.c.iii.c.iii.c.iii.c.iii.b Design Note (2026-03-13)

## Scope

- TODO leaf: `2.6.c.iv.d.iv.c.iv.c.iii.c.iii.c.iii.c.iii.b`
- Objective: rerun strict single-lane `fragilec` build-only replay and enforce blocker non-increase versus `2.6.c.iii` baseline.
- Estimated implementation size: `< 500 LOC` (verification/doc leaf).

## Plan

1. Rebuild release `fragilec` to keep replay runs deterministic.
2. Run strict build-only replay on a fresh run root.
...
- verification-only leaf using existing generic behavior
...
Leaf `2.6.c.iv.d.iv.c.iv.c.iii.c.iii.c.iii.c.iii.b` is complete and confirms no blocker-severity/E0425 regression versus the `2.6.c.iii` baseline while strict replay remains timeout-bound on `src/rrr/base/misc.cpp`.
