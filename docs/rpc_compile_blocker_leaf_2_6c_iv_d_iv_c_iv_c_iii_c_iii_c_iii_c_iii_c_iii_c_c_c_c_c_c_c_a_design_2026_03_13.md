# RPC Compile Blocker Leaf 2.6.c.iv.d.iv.c.iv.c.iii.c.iii.c.iii.c.iii.c.iii.c.c.c.c.c.c.c.a Design (2026-03-13)

## Scope

Reduce repeated work in function-template type-argument inference by reusing a
single inferred non-type array-ref bound across repeated non-type template
parameter candidates, without changing inference semantics.

## Design Rationale

- `infer_fn_template_type_args` already detects non-type template parameter
  candidates.
- Before this leaf, each non-type candidate repeated the same scan over
  `params` + `instantiated_args` to infer NTTP array-ref bounds.
- The scan result is independent of candidate index, so this leaf computes it
  once (`inferred_non_type_array_ref_arg`) and reuses it.
- This avoids repeated scans in a timeout-sensitive codegen region while
...
- Preserve existing inference ordering:
  - explicit parameter-driven type inference first
  - shared NTTP array-ref inference fallback for non-type candidates
  - return-type fallback when still unresolved
