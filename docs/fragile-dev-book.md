...
- Record struct emission now aliases a conservative allowlist of lifetime-free Rusty wrapper records directly to normalized Rust std surfaces (for example `rusty::Arc<T>` -> `std::sync::Arc<T>`, `rusty::Option<T>` -> `std::option::Option<T>`, `rusty::Result<T, E>` -> `std::result::Result<T, E>`, `rusty::Vec<T>` -> `std::vec::Vec<T>`) instead of emitting opaque wrapper structs; aliasing is rejected if any nested `rusty::` path remains after normalization, and lifetime-sensitive wrappers (`Ref`/`RefMut`) stay excluded.
