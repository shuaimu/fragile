...
- Unqualified Rusty wrapper spellings introduced by `using namespace rusty...` are normalized too for direct std-equivalent surfaces (for example `JoinHandle<T>`, `Condvar`, `Unit`, `RecvError`, `TryRecvError`) so downstream aliases/fields do not leak unresolved bare wrapper names.
