# RPC Compile Blocker Leaf 2.6.c.iv.d.iv.c.iv.c.iii.c.iii.c.iii.c.iii.c.iii.c.b Design Note (2026-03-13)
...
`2.6.c.iv.d.iv.c.iv.c.iii.c.iii.c.iii.c.iii.c.iii.c.a`.
...
severity or `E0425` count versus the fixed baseline
`2.6.c.iii` inventory manifest.
...
python3 scripts/mako_rpcbench_harness.py \
  --run-root /tmp/fragile_rpc_leaf_2_6c_iv_d_iv_c_iv_c_iii_c_iii_c_iii_c_iii_c_iii_c_b_build_only_20260313_v1 \
  --lanes fragilec \
  --build-only \
  --jobs 4 \
...
- full suite parity retained:
  - `cargo test --workspace --all-targets`:
    `fragile-clang` lib `741` passed / `46` failed (failure count unchanged)
  - `python3 -m unittest discover -s tests/python -p 'test_*.py'`:
    `OK (29 ran, 1 skipped)`
