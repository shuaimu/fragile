...
Current status: `crc32.c` replay compile typing is fixed (`wrapping_shl` lhs typing), and `.c` replay units now parse in C mode (the `deflate.c` `register` parser rejection is cleared). Real-world OBJZ replay now advances to `deflate.o` compile and fails on a new codegen issue: `configuration_table` emits positional initializer syntax (`config { ... }`) that is invalid for Rust struct literals. The next subleaf addresses that initializer translation, then reruns full `OBJZ` object replay.
