...
Current status: `crc32.c` replay compile typing is fixed (`wrapping_shl` lhs typing), `.c` replay units now parse in C mode (the `deflate.c` `register` parser rejection is cleared), and typedef-backed `configuration_table` aggregate initialization now emits named Rust fields instead of positional `config { ... }` syntax. Real-world OBJZ replay now advances further in `deflate.o` and fails on another codegen issue: chained C integer-bool comparisons are emitted as invalid Rust (`a != b != 0`). The next subleaf addresses chained comparison lowering, then reruns full `OBJZ` object replay.
