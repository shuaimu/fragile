...
fn print_node_tree(ctx: &fragile_ast_exporter::clang_ast::AstContext, node_id: u64, depth: usize) {
    ...
    if let Some(node) = ctx.ast_nodes.get(&node_id) {
        ...
        for child_opt in &node.children {
            if let Some(child_id) = child_opt {
                print_node_tree(ctx, *child_id, depth + 1);
            }
        }
    }
}
