...
fn main() {
    ...
    println!("=== Unordered Map Test ===");

    let temp_dir = std::path::Path::new("/tmp/fragile_unordered_map_test");
    let _ = fs::create_dir_all(&temp_dir);

    // Write C++ source to temp file for LibTooling
    let cpp_path = temp_dir.join("test_unordered_map.cpp");
    ...
    let rs_path = temp_dir.join("test_unordered_map.rs");
    ...
    let run_output = Command::new(temp_dir.join("test_unordered_map"))
        .output()
        .expect("Failed to run binary");

    println!("Exit code: {:?}", run_output.status.code());
    if run_output.status.success() {
        println!("✓ Unordered map test PASSED!");
    } else {
        println!("✗ Unordered map test FAILED!");
        std::process::exit(1);
    }
}
