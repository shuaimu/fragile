...
fn map_unit_payload_template_arg_to_rust(arg: &str) -> String {
...
fn map_single_template_alias_to_std_with_unit_payload_fallback(
    spelling: &str,
    alias_prefix: &str,
    std_path: &str,
) -> Option<String> {
    ...
    let mapped = map_unit_payload_template_arg_to_rust(&args[0]);
    Some(format!("{}<{}>", std_path, mapped))
}
        ...
        match self {
            ...
            CppType::Named(name) => {
                ...
                match normalized_name {
                    ...
                    // Some degraded parser lanes can append `__memory_order_modifier`
                    // fragments to byte spellings; normalize those back to `byte`.
                    "std::byte"
                    | "std::byte::__memory_order_modifier"
                    | "std_byte___memory_order_modifier"
                    | "std___byte___memory_order_modifier"
                    | "byte___memory_order_modifier" => "byte".to_string(),
                    // C++11 memory_order - map to the generated memory_order enum
                    "std::memory_order" => "memory_order".to_string(),
                    // libc++ internal memory-order modifier enum spellings appear in
                    // atomic helper signatures and should stay primitive.
                    "std::__memory_order_modifier"
                    | "std::__1::__memory_order_modifier"
                    | "std::__2::__memory_order_modifier"
                    | "std::__ndk1::__memory_order_modifier"
                    | "std___memory_order_modifier" => "i32".to_string(),
...
mod tests {
    ...
    fn test_rusty_thread_and_mpsc_types_map_to_std_library_types() {
        ...
        assert_eq!(
            CppType::Named("std::sync::mpsc::Sender<void>".to_string()).to_rust_type_str(),
            "std::sync::mpsc::Sender<()>"
        );
        assert_eq!(
        ...
        assert_eq!(
            CppType::Named("Receiver<void>".to_string()).to_rust_type_str(),
            "std::sync::mpsc::Receiver<()>"
        );
        assert_eq!(
        ...
        assert_eq!(
            CppType::Named("SyncSender<void>".to_string()).to_rust_type_str(),
            "std::sync::mpsc::SyncSender<()>"
        );
        assert_eq!(
        ...
        assert_eq!(
            CppType::Named("TrySendError<void>".to_string()).to_rust_type_str(),
            "std::sync::mpsc::TrySendError<()>"
        );
        assert_eq!(
    ...
    fn test_normalize_rusty_type_alias_to_std_maps_wrappers_and_preserves_non_rusty_paths() {
        ...
        assert_eq!(
            normalize_rusty_type_alias_to_std("std::sync::mpsc::Sender<void>"),
            "std::sync::mpsc::Sender<()>"
        );
        assert_eq!(
        ...
        assert_eq!(
            normalize_rusty_type_alias_to_std("Receiver<void>"),
            "std::sync::mpsc::Receiver<()>"
        );
        assert_eq!(
        ...
        assert_eq!(
            normalize_rusty_type_alias_to_std("SyncSender<void>"),
            "std::sync::mpsc::SyncSender<()>"
        );
        assert_eq!(
        ...
        assert_eq!(
            normalize_rusty_type_alias_to_std("TrySendError<void>"),
            "std::sync::mpsc::TrySendError<()>"
        );
        assert_eq!(
    ...
    #[test]
    fn test_memory_order_modifier_and_byte_suffix_aliases_normalize_to_expected_lanes() {
        assert_eq!(
            CppType::Named("std::__memory_order_modifier".to_string()).to_rust_type_str(),
            "i32"
        );
        assert_eq!(
            CppType::Named("std___memory_order_modifier".to_string()).to_rust_type_str(),
            "i32"
        );
        assert_eq!(
            CppType::Named("byte___memory_order_modifier".to_string()).to_rust_type_str(),
            "byte"
        );
        assert_eq!(
            CppType::Named("std::byte::__memory_order_modifier".to_string()).to_rust_type_str(),
            "byte"
        );
    }
