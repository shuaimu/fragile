...
fn test_real_world_tinyxml2_make_test_command_subset_replay_fragile() {
    ...
    assert!(
        err.contains("make-test command replay failed at command 1 with status 167"),
    ...
    assert_eq!(
        ...
        167,
        "current blocker should surface as non-crashing status 167 on replay command 1"
    );
    let replay_stderr = fs::read_to_string(log_dir.join("make_test_replay_01.stderr"))
    ...
    assert_eq!(
        fail_lines[0],
        "[fail] Formatted error string [Error=XML_ERROR_PARSING_ATTRIBUTE ErrorID=7 (0x7) Line number=3: XMLElement name=wrong][XML_ERROR_UNKNOWN]",
    ...
    assert!(
        replay_stdout.contains("Pass 302, Fail 167"),
