...
fn test_real_world_tinyxml2_make_test_command_subset_replay_fragile() {
    ...
    assert_eq!(
        ...
        127,
        "current blocker should surface as non-crashing status 127 on replay command 1"
    );
    let replay_stderr = fs::read_to_string(log_dir.join("make_test_replay_01.stderr"))
    ...
    assert_eq!(
        fail_lines[0],
        "[fail] GetText() normal use. [This is  text][text]",
    ...
    assert!(
        replay_stdout.contains("Pass 342, Fail 127"),
