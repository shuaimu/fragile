...
fn test_real_world_tinyxml2_make_test_command_subset_replay_fragile() {
    ...
    assert_eq!(
        ...
        9,
        "current blocker should surface as non-crashing status 9 on replay command 1"
    );
    let replay_stderr = fs::read_to_string(log_dir.join("make_test_replay_01.stderr"))
    ...
    assert!(
        fail_lines[0].starts_with("[fail] Test mismatched elements. [true][false]"),
    ...
    assert!(
        replay_stdout.contains("Pass 455, Fail 9"),
