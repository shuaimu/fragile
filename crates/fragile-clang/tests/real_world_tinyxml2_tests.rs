...
fn test_real_world_tinyxml2_make_test_command_subset_replay_fragile() {
    ...
    assert!(
        err.contains("make-test command replay failed at command 1 with status 26"),
    ...
    assert_eq!(
        ...
        26,
        "current blocker should surface as non-crashing status 26 on replay command 1"
    );
    let replay_stderr = fs::read_to_string(log_dir.join("make_test_replay_01.stderr"))
    ...
    assert!(
        fail_lines[0].starts_with("[fail] Loading an empty file and ErrorName as string"),
    ...
    assert!(
        replay_stdout.contains("Pass 438, Fail 26"),
