...
fn test_real_world_tinyxml2_make_test_command_subset_replay_fragile() {
    ...
    assert!(
        err.contains("make-test command replay failed at command 1 with status 57"),
    ...
    assert_eq!(
        ...
        57,
        "current blocker should surface as non-crashing status 57 on replay command 1"
    );
    let replay_stderr = fs::read_to_string(log_dir.join("make_test_replay_01.stderr"))
    ...
    assert_eq!(
        fail_lines[0],
        "[fail] Element names with lead digit fail to parse. [true][false]",
    ...
    assert!(
        replay_stdout.contains("Pass 416, Fail 57"),
