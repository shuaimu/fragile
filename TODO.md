...
- [ ] M9.2 Run full strict runtime replay and capture deterministic runtime manifests.
  ...
  - [ ] M9.2.c Execute one passing strict runtime replay run-root for fragilec lane (`test_rpc` + rpcbench server/client). **WIP on claude/parser branch.** Root cause: default fragilec path is `target/debug/fragilec` (debug binary, ~87MB, 10x slower than release); build timeout of 1800s is insufficient for mako with debug fragilec. Fix: switch default to `target/release/fragilec`, increase build timeout, rebuild release binary. Current blocker (2026-03-19): deterministic replay run-root `/tmp/fragile_m9_2_strict_runtime_replay_20260319T093009Z_p683647` records `lane_fragilec_build_status=124`, `lane_fragilec_failure_class=build_timeout`, `lane_fragilec_test_rpc_status=-1`, `runtime_all_trials_passed=false` with complete artifact contract coverage (`missing_required_artifact_count=0`).
