...
- [ ] Make transpiled build pass zlib test commands used by upstream `make test`.
  ...
  - [ ] Fix strict link input/symbol/flag gaps until make-test replay command #1 runs successfully under Fragile-linked binaries.
    ...
    - [ ] Re-run make-test command-subset replay and advance command #1 to status `0`.
      - Analysis (2026-02-20): this remains larger than a single <500 LOC leaf because replay determinism, ignored-test expectations, and runtime fixes must be closed in sequence.
      ...
      - [ ] Fix command #1 data-integrity failure (`echo hello world | ./minigzip | ./minigzip -d` reports `incorrect data check`).
