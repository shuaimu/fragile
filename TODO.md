...
- [ ] Make transpiled build pass zlib test commands used by upstream `make test`.
  ...
  - [ ] Fix strict link input/symbol/flag gaps until make-test replay command #1 runs successfully under Fragile-linked binaries.
    ...
    - [ ] Export required non-static C global data symbols with unmangled C ABI names in generated Rust (close `_dist_code`/`_length_code` strict-link gaps).
    - [ ] Re-run strict required-link replay and confirm all required outputs (`example`, `minigzip`, `examplesh`, `minigzipsh`, `example64`, `minigzip64`) link successfully with non-empty binaries.
