    ...
    - [ ] Replay `OBJZ` units through Fragile to `.o` outputs and validate object completeness.
      ...
      - [ ] Re-run full `OBJZ` replay and confirm all expected OBJZ objects compile and are non-empty.
        ...
        - [ ] Fix remaining `deflate` type/union field/codegen mismatches in replay output (`freq` union field access, integer width mismatches, pointer arithmetic typing).
          ...
          - [ ] Fix enum/integer normalization in `infback` replay output (`inflate_mode` / `codetype` assignments and call arguments).
            - Current first blocker class (`/tmp/fragile_real_world_zlib_fragile_objz_objects/driver_logs/rustc_objz_infback_o.stderr`): integer literals emitted where enum-typed lvalues/params are required (e.g., `(*state).mode = 16191`, `inflate_table(0, ...)`).
