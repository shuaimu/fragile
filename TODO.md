...
- [ ] 6) Numeric/sign/enum lowering issues. Status: OPEN (triaged with locked replay markers). Evidence: strict RapidJSON replays now lock item-6 numeric/sign marker presence (`E0600` unary negation on `u128`, `E0604` `u8`→`char` cast misuse, `E0618` constant-as-function calls, and `__gv___pow10_128` literal typing mismatch), confirming the class remains active and providing deterministic acceptance criteria for follow-up fixes.
...
- [ ] 6.2) Normalize `u128` power-table and unary-negative lowering in generated numeric helper constants (remove unsigned-negation and mixed-width literal typing diagnostics).
- [ ] 6.3) Fix numeric helper call-shape regressions that emit constant/statics as callable symbols (`expected function, found i64/{integer}`) in strict RapidJSON captures.
- [ ] 6.4) Re-run strict `filterkeydom` + strict no-tests CMake captures and require item-6 marker set absence in first-failure stderr before marking ordered ledger item 6 CLEARED.
