...
- [ ] G5 Runtime Gate (deferred RPC): `test_rpc` exits successfully; `rpcbench` server/client run without crash/hang. Blocked: M9.2.c runtime replay currently blocked by parser-output mapping-completeness build failures in strict fragilec lane.
...
- [ ] M9.2 Run full strict runtime replay and capture deterministic runtime manifests.
  ...
  - [ ] M9.2.c Execute one passing strict runtime replay run-root for fragilec lane (`test_rpc` + rpcbench server/client). Current blocker (2026-03-19): mapping-completeness diagnostics resolved; strict lane now fails with downstream rustc/codegen errors in `rrr/base/{debugging,misc,basetypes,logging}.cpp` (syntax errors, duplicate type defs E0428, unresolved types, missing headers).
    ...
    - [ ] M9.2.c.iv Re-run strict runtime replay until `lane_fragilec_build_status=0`, `lane_fragilec_test_rpc_status=0`, `lane_fragilec_failure_class=none`, and runtime trial artifacts all pass. Current blocker snapshot (2026-03-19): mapping-completeness diagnostics resolved; remaining failures are downstream rustc/codegen errors (syntax, E0428 duplicate defs, unresolved types, missing headers).
      ...
      - [ ] M9.2.c.iv.d Re-run strict replay after mapping-completeness closure and resolve any newly exposed downstream rustc/codegen/runtime blockers generically. **WIP on claude/parser branch.** Current blocker taxonomy (2026-03-19 compile evidence, post d.1 fix): `debugging.cpp` (491 rustc errors: 200 E0308 type mismatches from ios_base fmtflags→u128 mapping, 54 E0605 casts, 38 E0599 methods on u128, 19 E0609 fields on u128, 8 E0433 _Range_chk unresolved, 16 E0425 missing throw functions, etc.), `logging.cpp` (clang parse failure: `threading.hpp` includes `rusty/*.hpp` headers not in include path), `misc.cpp` (484 rustc errors: same pattern as debugging.cpp). `basetypes.cpp` unresolved-type invariant for `byte___memory_order_modifier` is addressed by d.1 and awaits strict replay verification in d.4.
        - [x] M9.2.c.iv.d.1 Fix `basetypes.cpp` unresolved-type invariant for `byte___memory_order_modifier`. Done 2026-03-19. Evidence: `crates/fragile-clang/src/types.rs` now normalizes `byte___memory_order_modifier`/`std::byte::__memory_order_modifier` to `byte` and `std::__memory_order_modifier` spellings to primitive `i32`; regression `test_memory_order_modifier_and_byte_suffix_aliases_normalize_to_expected_lanes` added in `types.rs`.
        - [ ] M9.2.c.iv.d.2 Fix `logging.cpp` clang parse failure from missing `rusty/*.hpp` headers in `threading.hpp`. Root cause: `threading.hpp` includes `rusty/result.hpp`, `rusty/option.hpp`, `rusty/unsafe_cell.hpp` — these are Rust-interop headers that exist in a separate include path not passed to fragilec. Estimated: ~20-40 LOC (add include path resolution or parse-error tolerance for missing project-specific headers).
        - [ ] M9.2.c.iv.d.3 Fix `debugging.cpp`/`misc.cpp` ios_base fmtflags type mapping (u128 → proper integer type). Root cause: `std::ios_base::fmtflags` and related iostream types mapped to `u128` instead of integer/bitflag types, causing 200+ E0308 type mismatches per file. Estimated: ~50-100 LOC (type normalization in parse.rs/types.rs).
        - [ ] M9.2.c.iv.d.4 Fix `debugging.cpp`/`misc.cpp` missing STL helper functions (`__throw_out_of_range`, `__throw_invalid_argument`, `_Range_chk`). Root cause: internal libc++ helpers referenced but not defined in transpiled output. Estimated: ~20-40 LOC (add to preamble or map to existing equivalents).
        - [ ] M9.2.c.iv.d.5 Fix remaining `debugging.cpp`/`misc.cpp` codegen errors (E0117 orphan impls, E0277 missing trait impls, E0061 argument count mismatches, etc.). Estimated: ~50-200 LOC depending on root causes.
...
- [ ] M9.A2 `rpcbench` server/client runtime gate is green. Blocked: M9.2.c strict replay currently fails in build phase due parser-output mapping-completeness diagnostics tracked in M9.2.c.iv before runtime steps.
