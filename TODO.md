...
- [ ] 3) Placeholder fallback for required rapidjson template types. Status: OPEN. Evidence: strict `filterkeydom` replay no longer reports the prior `FilterKeyReader_FileReadStream::new_0` / `GenericDocument_UTF8_::Populate` / `GenericDocument_UTF8_::Accept` API-hole markers, but broader placeholder degradation checks remain open under 3.2.c and 3.3.
