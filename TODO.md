...
- [ ] M9.2 Run full strict runtime replay and capture deterministic runtime manifests.
  ...
  - [ ] M9.2.c Execute one passing strict runtime replay run-root for fragilec lane (`test_rpc` + rpcbench server/client). Current blocker (2026-03-19): mapping completeness failures resolved; 4 remaining rustc codegen failures in `rrr/base/*.cpp` (struct init syntax, duplicate type definitions, unresolved type invariant).
    ...
    - [ ] M9.2.c.iv Re-run strict runtime replay until `lane_fragilec_build_status=0`, `lane_fragilec_test_rpc_status=0`, `lane_fragilec_failure_class=none`, and runtime trial artifacts all pass. Remaining blockers: 4 rustc codegen failures in `rrr/base/misc.cpp` (struct init syntax error, duplicate `basic_istream_char` type), `rrr/base/basetypes.cpp` (unresolved `byte___memory_order_modifier` type invariant), `rrr/base/debugging.cpp` and `rrr/base/logging.cpp` (rustc compile errors).
