...
- [ ] 1.4) Replace the temporary narrowed parser ignore with a real semantic-fidelity fix once downstream compile/codegen blockers are cleared.
  ...
  - [ ] 1.4.d) Re-run strict `filterkeydom` and strict no-tests CMake capture to confirm parser no longer depends on RapidJSON ignore-pattern config for this class.
