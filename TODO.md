...
- [ ] 3) Placeholder fallback for required rapidjson template types. Status: OPEN. Evidence: strict `filterkeydom` replay still reports RapidJSON API-hole diagnostics from degraded placeholder surfaces (for example missing `FilterKeyReader_FileReadStream::new_0`, `GenericDocument_UTF8_::Populate`, `GenericDocument_UTF8_::Accept`).
...
- [ ] 3.2) Stop degrading `rapidjson::GenericDocument`/reader/writer concrete instantiation paths to placeholder-only structs when full definitions are available in parsed AST output.
  ...
  - [ ] 3.2.b) Prioritize concrete `GenericDocument`/reader/writer instantiation emission over placeholder fallback when complete specialization field/method data is available in AST/libtooling outputs.
  - [ ] 3.2.c) Add replay evidence that degraded placeholder structs for active `filterkeydom` document/reader/writer paths are no longer emitted when concrete instantiations are present.
- [ ] 3.3) Add AST-codegen regression(s) for representative rapidjson concrete instantiation(s) to ensure concrete structs/impl methods emit and placeholder fallback is skipped for active code paths.
- [ ] 3.4) Re-run strict `filterkeydom` + strict no-tests CMake captures; require placeholder API-hole markers from 3.1 to disappear before marking ordered ledger item 3 CLEARED.
