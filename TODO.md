...
- [ ] 5) Cast/decay/call-shape lowering bugs. Status: OPEN (partially mitigated). Evidence: constructor-path and degraded non-constructor array-decay regressions are now covered/fixed in 5.1/5.2 (including mutable-pointer decay in fallback call paths), but strict transpiled output still surfaces remaining pointer/value call-shape mismatches tracked by 5.3/5.4.
