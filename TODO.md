  ...
  - [ ] 5.4.c) Fix LibTooling-primary regressions until parity with current strict baseline (Effort: M, Risk: High).
    ...
    - [ ] 5.4.c.ii) Re-run ignored strict backend matrix and require LibTooling-primary build/runtime parity with baseline (CMake no-tests build + condense/pretty runtime checks) before cutover.
      ...
      - [ ] 5.4.c.ii.3) Fix the top-ranked `tutorial.cpp` LibTooling blocker from 5.4.c.ii.2 and require strict backend-matrix `libtooling` replay to complete without `compile_timeout`.
        ...
        - [ ] 5.4.c.ii.3.c) Resolve strict backend-matrix `libtooling` E0425 delta regression introduced after 5.4.c.ii.3.a and restore the non-increase gate against the previous baseline snapshot.
