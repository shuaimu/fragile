...
- [ ] M9.2 Run full strict runtime replay and capture deterministic runtime manifests.
  ...
  - [ ] M9.2.c Execute one passing strict runtime replay run-root for fragilec lane (`test_rpc` + rpcbench server/client). Build timeout blocker fixed 2026-03-19: switched all orchestration scripts from `target/debug/fragilec` to `target/release/fragilec` and increased build timeout from 900s to 3600s; 7 Rust + 6 Python regression tests validate the fix. Remaining: execute the actual runtime replay run with release binary to produce a passing manifest (requires `cargo build --release -p fragile-cli --bin fragilec`). **WIP on claude/parser branch 2026-03-19.**
