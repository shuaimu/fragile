...
- [ ] 6) Numeric/sign/enum lowering issues. Status: OPEN (6.2 landed; residual markers locked). Evidence: strict RapidJSON replays now assert item-6.2 cleared-marker absence (`E0600` unary negation on `u128`, mixed-width `__gv___pow10_128` `10u64` typing, and `E0080` u128 const-overflow in pow10 table) while retaining residual-marker presence (`E0604` `u8`→`char` cast misuse and `E0618` constant-as-function calls), keeping deterministic acceptance criteria for 6.3/6.4.
...
- [ ] 6.3) Fix residual numeric helper diagnostics in strict RapidJSON captures (`E0604` pointer-to-`char` cast misuse and `E0618` constant/statics emitted as callable symbols).
- [ ] 6.4) Re-run strict `filterkeydom` + strict no-tests CMake captures and require residual item-6 marker absence (`E0604`, `E0618`) in first-failure stderr before marking ordered ledger item 6 CLEARED.
