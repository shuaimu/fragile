...
- [ ] 7) Entrypoint correctness residual (`main` rollback/drop). Status: OPEN (partially mitigated). Evidence: Phase 0 removed shim-only false-positive links; local strict fragilec-driver no-stl fixture now compiles/runs `condense` and `pretty` with expected JSON output, while real-world strict no-stl and full strict no-tests CMake runtime validation remain open in Phase 3.
